...
class VideoRecord(object):
    ...
    @property
    def label(self) -> int | list[float]:
        # just one label_id
        if len(self._data) == 4:
            return ast.literal_eval(self._data[3])
        # sample associated with multiple labels
        else:
            return [float(label_id) for label_id in self._data[3:]]
...
class VideoFrameDataset(torch.utils.data.Dataset):
    ...
    Note:
        ...
        be ``home\data\datasetxyz\videos\``, inside which a ``VIDEO_FOLDER_PATH``
    ...
    def _get(self, record: VideoRecord, frame_start_indices: 'np.ndarray[int]') -> Union[
        ...
        width_ratio = image_width / self.resize_shape
        height_ratio = image_height / self.resize_shape

        labels = []
        for frame in frame_indices:
            labels.append(record.label[frame-1])
        labels = torch.FloatTensor(labels)

        # normalize the labels for the image resizing
        # x = x / width_ratio, width = width / width_ratio
        # y = y / width_ratio, height = height / height_ratio
        div = torch.Tensor([1, width_ratio, height_ratio, width_ratio, height_ratio])
        normalized_labels = torch.div(labels, div)
        return images, normalized_labels
